...
\begin{starterprogram}[_082507357]{リスト1.1: }\seqsplit{  const http = require('http');

  const hostname = '127.0.0.1';
  const port = 3000;

  const server = http.createServer((req, res) =\textgreater{} \{
    res.statusCode = 200;
    res.setHeader('Content{-}Type', 'text/plain');
    res.end('Hello World');
    \});

    server.listen(port, hostname, () =\textgreater{} \{
      console.log(`Server running at http://\textdollar{}\{hostname\}:\textdollar{}\{port\}/`);
    \});
}\end{starterprogram}
...
\def\startercodeblockfontsize{}
\begin{starterterminal}[_161885662]{リスト1.2: app.jsを実行}\seqsplit{  \textgreater{} node app.jp
  Server running at http://127.0.0.1:3000/
}\end{starterterminal}

ブラウザを開き、http://127.0.0.1:3000へアクセスすると、ブラウザに実行結果が表示されます。

\begin{reviewimage}%%01_00browser_helloWorld
\includegraphics[width=1.0\maxwidth]{./images/01-createDevEnv/01_00browser_helloWorld.png}%
\reviewimagecaption{app.jsの実行}
...
ここでダウンロード可能なのは、「14.17.0 LTS(Long Term Support)推奨版」と「16.3.0最新版」\footnote{2021/06/10現在}の2つがあります。
...
\def\startercodeblockfontsize{}
\begin{starterterminal}[_217808539]{リスト1.3: nvmのインストール}\seqsplit{  \textgreater{}curl {-}o{-} https://raw.githubusercontent.com/nvm{-}sh/nvm/v0.37.2/install.sh \textbar{} bash}\end{starterterminal}
...
\paragraph*{Windows}
nvm{-}windowsのリリースページ:\url{https://github.com/coreybutler/nvm-windows/releases/tag/1.1.7}より、
...
インストールは、Mac、Windowsとも、「nvm install インストールするバージョン番号」でインストールできます。
...
\def\startercodeblockfontsize{}
\begin{starterterminal}[]{npmのバージョン確認}\seqsplit{\textdollar{} \textgreater{} npm {-}v
6.14.6}\end{starterterminal}

yarnをインストールするために、この「npm」コマンドを使用します。

\def\startercodeblockfontsize{}
\begin{starterterminal}[]{yarnのインストール}\seqsplit{\textdollar{} \textgreater{} npm install {-}g yarn}\end{starterterminal}
...
HTML・CSS・JavaScriptを使用して、Windows、Mac、Linuxのマルチプラットフォームで動作するアプリケーションを開発することができます。
...
デバッグなども行えるため、Web開発を行う上出は、事実上の標準と言っても良いかもしれません。有料では、Jetbrains社:\url{https://www.jetbrains.com/}の
...
VSCodeは、プラグイン形式で拡張機能を追加することができます。
...
\subsubsection*{Bracket Pair Colorizer}
\keeplastskip{
  \label{sec:1-3-2-1}
  \par\nobreak
}
...
\begin{reviewimage}[H]%%01_05vscodeExtension_bracketsPair
\includegraphics[width=1.0\maxwidth]{./images/01-createDevEnv/01_05vscodeExtension_bracketsPair.png}%
\reviewimagecaption{Beautify}
\label{image:01-createDevEnv:01_05vscodeExtension_bracketsPair}
\end{reviewimage}
...
===　Google Chromeの拡張機能
こちらも、VSCodeと同様に拡張機能を追加することで、更に便利に使うことができます。

React、Reduxの開発では、以下の拡張機能は必須と言っても良いほどです。

拡張機能のインストールは、chromeウェブストアで検索してください。

chromeウェブストア:\url{https://chrome.google.com/webstore/category/extensions?hl=ja}

\begin{reviewimage}[H]%%01_13chromeWebstore
\includegraphics[width=1.0\maxwidth]{./images/01-createDevEnv/01_13chromeWebstore.png}%
\reviewimagecaption{chromeウェブストア}
...
この拡張機能を使うと、Google ChromeのDevToolsにComponentsタブが作成され、Props、Stateを確認することができます。
...
後ほど、Reduxの章で改めて説明しますが、「タイムトラベルデバッグ(実行されたアクションを遡る)」が簡単にできるようになります。
